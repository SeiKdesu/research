...
bset_pop = 0
msees=[]
pop = list(range(66, 200)) 
for pop_size in range(66,200):
    # 初期集団の生成
...
plt.plot(pop,  msees, '-r', label='mse')
plt.title("pop size and MSE")

plt.savefig(f'rosenbrock/{name}_rbf_original_contour.pdf')  # 保存
plt.close()
print('これがbestなpopとbestなmseです',bset_pop,best_mse)
