...
class Ackley(Problem):
    def _initialize(self):
        self.options.declare("name", "Ackley", types=str)

    def _setup(self):
        # Define the boundaries for the Ackley function
        self.xlimits[:, 0] = -5.0  # Lower bound
        self.xlimits[:, 1] = 5.0   # Upper bound
    ...
    def _evaluate(self, x, kx):
        ...
        if kx is None:
            ...
            term1 = -a * np.exp(-b * np.sqrt(sum1 / nx))
            term2 = -np.exp(sum2 / nx)
            
            y[:, 0] = term1 + term2 + a + np.exp(1)
        else:
            raise NotImplementedError("Derivatives are not implemented for Ackley function.")

        return y
import numpy as np
from smt.utils.misc import compute_rms_error

# from smt.problems import Ackley
...
"""## Ackley Function  in dimension N
...
plt.ylabel("x2")
plt.legend()
plt.savefig('Ackley function.png')
plt.close()
...
# グラフの表示
plt.savefig('ackley_contour.pdf')
plt.close()
...
if plot_status:
    ...
    plt.savefig('Kriging_DixonPrice_contour.pdf')
...
plt.legend()
plt.title("Ackley function with the DOE points and predicted values")

plt.savefig('Ackley function with the DOE points and predicted values.png')
...
plt.title(" Ackley Surrogate Model with the 99% confidence interval ")
plt.xlabel("x1")
plt.ylabel("x2")
plt.legend()
plt.savefig('Ackley Surrogate Model with the 99% confidence interva.png')
...
plt.title("Ackley surrogate model error")
plt.xlabel("x1")
plt.ylabel("x2")
plt.savefig('Ackley surrogate model error.png')
