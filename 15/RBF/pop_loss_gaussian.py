...
def objective_function(x,dim):
    values=[]
    for tmp in x:
        tmp1 = Rosenbrock(tmp,dim)
        tmp2 = Rosenbrock1(tmp,dim)
...
for i in range(20,100):
    ...
    # xlim=np.array([-5.0,5.0],[-5.0,5.0])
    sampling = LHS(xlimits=np.array([[-5.0,5.0]]*ndim), criterion="ese", random_state=1)
    xt = sampling(ndoe)
    # Compute the outputs
    yt = objective_function(xt,ndim)

    # Construction of the validation points
    ntest = 200  # 500
    sampling = LHS(xlimits=np.array([[-5.0,5.0]]*ndim), criterion="ese", random_state=1)
    xtest = sampling(ntest)
    ytest = objective_function(xtest,ndim)
...
plt.legend()
plt.grid(True)
plt.savefig(f"{name}/pop_and_loss_ga.pdf")