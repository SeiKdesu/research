...
def devide_deminsion(pp,labels):
    ...
    #     if value % 2 == 0:  # 偶数のときにlist0_countを増やす
    #         list0_count += 1
    ...
    mal_lists = [np.array(globals()[f'mal_list{i}']) for i in range(dim)]
...
def train(dt):
    ...
    pp_list = torch.tensor(pp_list ,dtype=torch.float).to(device)
    ...
    for pp_item in pp_list[:dim]:  # 最初の 6 要素に対して処理
        all_predictions_devide += (predict_model(pp_item))
...
#         formatted_weight_data = np.vstack([new_array, formatted_weight_data])
#     ic(formatted_weight_data.shape)
