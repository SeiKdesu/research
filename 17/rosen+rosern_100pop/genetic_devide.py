    ...
    def run(self,labels):
        ...
        while t<=self.iterate:
            ...
            for k in range(self.par_s, self.pop_s, 2):
                ...
                obj=self.sim(ch1,labels)
                solo[self.dim]=obj
                pop[k]=solo.copy()                
                solo[: self.dim]=ch2.copy()                
                obj=self.sim(ch2,labels)               
                solo[self.dim]=obj
                pop[k+1]=solo.copy()
        #############################################################       
            t+=1
        ...
        if self.convergence_curve==True:
            ...
            plt.title('rosen+rosen 6dim devide')
            plt.savefig('rosen+rosern_6dim_devide_convergence.png')
...
###############################################################################     
    def evaluate(self,lables):
        ...
        self.temp = self.temp.reshape(-1,dim)
        population = np.array(self.temp)
        pop_devide = np.zeros((population.shape[0],6),dtype=float)


        
        pop_devide[:,lables] = population
        return self.f(pop_devide)
###############################################################################    
    def sim(self,X,lables):
        self.temp=X.copy()
        obj=None

        try:
            obj=func_timeout(self.funtimeout,lambda: self.evaluate(lables))
...
for i in range(count):
    ...
    local_best_pop = 0
    local_best_fit = 0
    model=geneticalgorithm(function=predict_surrogate,dimension=dim,variable_type='real',variable_boundaries=varbound)

    local_best_pop, local_best_fit   = model.run(labels_ga[i])
    global_best_pop.append(local_best_pop)
    global_best_fitness.append(local_best_fit)
global_best_pop = [item for sublist in global_best_pop for item in sublist]
global_best_pop = np.array(global_best_pop)
global_best_pop = global_best_pop.reshape(1,6)
ic(global_best_pop)
ic(global_best_fitness)
global_best_pop_obg = global_best_pop.squeeze()
ic(f(global_best_pop_obg))

ic(predict_surrogate(global_best_pop))
