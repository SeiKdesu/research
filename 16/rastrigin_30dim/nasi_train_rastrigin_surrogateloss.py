...
def devide_deminsion(pp,labels):
    ...
    # mal_list29=[]
    # mal_list30=[]
    mal_lists = [globals()[f'mal_list{i}'] for i in range(30)]
    list1_count =0
    # mal_list2=[]
    # list2_count =dim
    ...
    for count in range(dim):
        ...
        if classfication[count] == 6:
            mal_list6.append(count)
            list0_count += 1
        if classfication[count] == 7:
            mal_list7.append(count)
        if classfication[count] == 8:
            mal_list8.append(count)
            list0_count += 1
        if classfication[count] == 9:
            mal_list9.append(count)
        if classfication[count] == 10:
            mal_list10.append(count)
            list0_count += 1
        if classfication[count] == 11:
            mal_list11.append(count)
        if classfication[count] == 12:
            mal_list12.append(count)
            list0_count += 1
        if classfication[count] == 13:
            mal_list13.append(count)
        if classfication[count] == 14:
            mal_list14.append(count)
            list0_count += 1
        if classfication[count] == 15:
            mal_list15.append(count)
        if classfication[count] == 16:
            mal_list16.append(count)
            list0_count += 1
        if classfication[count] == 17:
            mal_list17.append(count)
        if classfication[count] == 18:
            mal_list18.append(count)
            list0_count += 1
        if classfication[count] == 19:
            mal_list19.append(count)
        if classfication[count] == 20:
            mal_list20.append(count)
            list0_count += 1
        if classfication[count] == 21:
            mal_list21.append(count)
        if classfication[count] == 22:
            mal_list22.append(count)
            list0_count += 1
        if classfication[count] == 23:
            mal_list23.append(count)
        if classfication[count] == 24:
            mal_list24.append(count)
            list0_count += 1
        if classfication[count] == 25:
            mal_list25.append(count)
        if classfication[count] == 26:
            mal_list26.append(count)
            list0_count += 1
        if classfication[count] == 27:
            mal_list27.append(count)
        if classfication[count] == 28:
            mal_list28.append(count)
            list0_count += 1
        if classfication[count] == 29:
            mal_list29.append(count)
        
    
    import numpy as np

    # mal_list0 から mal_list29 までのリストを np.array に変換
    mal_lists = [mal_list0, mal_list1, mal_list2, mal_list3, mal_list4, mal_list5, 
                mal_list6, mal_list7, mal_list8, mal_list9, mal_list10, mal_list11, 
                mal_list12, mal_list13, mal_list14, mal_list15, mal_list16, mal_list17, 
                mal_list18, mal_list19, mal_list20, mal_list21, mal_list22, mal_list23, 
                mal_list24, mal_list25, mal_list26, mal_list27, mal_list28, mal_list29]

    # 各 mal_list を np.array に変換
    for i in range(len(mal_lists)):
        mal_lists[i] = np.array(mal_lists[i])
    ...
    pp_list = []
    for i in range(dim):  # mal_list0 から mal_list5 まで処理する
        pp_list.append(keep_indices_as_nonzero(pp, mal_lists[i]))

    # pp_list[0] が pp1, pp_list[1] が pp2, ... と対応
...
def train(dt):
    ...
    all_prediction = torch.tensor(all_predictions_devide)
    loss_func=nn.L1Loss()
...
#########################################################################
    ...
    for i in range(dim):
        new_array = np.zeros(dim)  # 6 要素のゼロの配列を作成
        new_array[i] = num[i]  # num[i] を対応する位置にセット
        formatted_weight_data = np.vstack([new_array, formatted_weight_data])


    to_match = np.ones_like(dim)
    formatted_weight_data = np.vstack([formatted_weight_data,to_match])


    num1 = get_yt()
    new_array_y = np.zeros(dim)
    new_array_y[0] = num1[com][0]
    new_temp= np.array(new_array_y)
    ...
    dataset=Data(x=x,edge_index=edge_index,edge_attr=edge_attr,y=y,num_classes=dim+1)
