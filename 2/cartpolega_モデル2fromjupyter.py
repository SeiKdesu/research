...
def fitness(solution: Parameters, net: torch.nn.Sequential, render=True) -> float:
    ...
    while not done: #while Trueのようなもの。
        ...
        ob = torch.tensor(ob).float().unsqueeze(0)#pytrochの用意している型のことであり、イメージnumpyぽもの？obの観測データをpytrochの配列として、float型で行う。データはunsqueezeでtensorの次元を1次元増やす
        ...
        ob = binary_representation.float().to(device)
