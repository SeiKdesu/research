...
def trid(x,n):
   ...
   # x = np.arange(1, n + 1)
    ...
    return sum1 - sum2
def objective_function1(x,dim):
    # n_rosenbrock = 30
    n_dixon=30
    # n_powell=32
    # rosen_value = Rosenbrock(x[:n_rosenbrock], n_rosenbrock)
    dixon_value = dixon_price(x[:n_dixon],n_dixon)
...
def evaluate_population(population):
    return [objective_function1(individual,dim) for individual in population]
...
def genetic_algorithm(dim, max_gen, pop_size, offspring_size, bound):
    ...
    for generation in range(max_gen):
        ...
        population = sorted(population, key=lambda x: objective_function1(x,dim))[:pop_size]
