...
def dixon_price(x,n,dim):
    if dim==0:
        ...
        # n = len(x)
        # term1 = (x[0] - 1) ** 2
        # term2 = sum([i * (2 * x[i]**2 - x[i-1])**2 for i in [32,34,35,39,51,53,59]])
    elif dim==1:
        
        n = len(x)
        term1 = (x[0] - 1) ** 2
        term2 = sum([i * (2 * x[i]**2 - x[i-1])**2 for i in range(4,8)])
    elif dim==2:
        term1=0
        term2=0
        # n = len(x)
...
def objective_function(x,num):
    ...
    powell_value= Rosenbrock(x,n_rosenbrock,num)
...
# ルーレット選択
def roulette_wheel_selection(population, fitness,count):
    ...
    # re = population[current_best_fitness_index]
    ...
    for i in range(3):
        ...
        for generation in range(max_gen):
            ...
            while len(new_population) < offspring_size:
                count=0
                parent1 = roulette_wheel_selection(population, fitness,count)
                count += 1
                parent2 = roulette_wheel_selection(population, fitness,count)
                count += 1
                parent3 = roulette_wheel_selection(population, fitness,count)
                count += 1
...
print(objective_function3(best_individual,dim))
import matplotlib.pyplot as plt
